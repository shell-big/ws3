#!/usr/bin/env bash
# =============================================================================
# BlueRobotics bcm_27xx.sh (同梱版・Pi 4B 専用)
# 元リポジトリ: https://github.com/bluerobotics/blueos-docker
# パス        : install/boards/bcm_27xx.sh
#
# [変更点] spi0-led.dts をリモートから curl する代わりに、
#          同梱の scripts/overlays/spi0-led.dts を使用するよう変更。
# =============================================================================
...
# SCRIPT_DIR: このスクリプト自身があるディレクトリ
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

# Uses /boot/firmware directory if exists (Bookworm 以降)
...
# spi0-led デバイスツリーオーバーレイのコンパイルとインストール
# 同梱の .dts ファイルを使用（リモート curl を不要にする）
echo "- compile spi0 device tree overlay."
DTS_NAME="spi0-led"
DTS_LOCAL="$SCRIPT_DIR/overlays/$DTS_NAME.dts"

if [ -f "$DTS_LOCAL" ]; then
    echo "  Using bundled overlay: $DTS_LOCAL"
    dtc -@ -Hepapr -I dts -O dtb -o "$OVERLAYS_DIR/$DTS_NAME.dtbo" "$DTS_LOCAL"
else
    echo "  Bundled overlay not found, falling back to remote download."
    ...
    curl --retry 6 --max-time 15 --retry-all-errors --retry-delay 20 --connect-timeout 60 \
        -fsSL -o /tmp/$DTS_NAME "$ROOT/install/overlays/$DTS_NAME.dts"
